...
// Profil verisinin yapısını belirlemek için Profile arayüzünü oluşturuyoruz
interface Profile {
  username: string;
  first_name: string;
  last_name: string;
...
interface ProfileEditDialogProps {
  open: boolean;
  onClose: () => void;
  onSave: (updatedProfile: Profile) => void; // Profil güncelleme fonksiyonu
}
...
const ProfileEditDialog: React.FC<ProfileEditDialogProps> = ({ open, onClose, onSave }) => {
  ...
  // Kullanıcının ID'sini ve access token'ını localStorage'dan al
  const user = JSON.parse(localStorage.getItem('user') || '{}');
  const userId: string | null = user ? user.id : null;
  const accessToken: string | null = localStorage.getItem('access_token'); 

  // Profil verisini sunucudan al (GET isteği)
  useEffect(() => {
    if (open && userId && accessToken) {
      setLoading(true);
      axios.get(`${prodUrl}/customerauth/user/profile/${userId}`, {
        headers: {
        ...
        .then(response => {
          if (response.status === 200) {
            setUpdatedProfile(response.data); // Profil verisini al ve state'e kaydet
          } else {
  ...
  // Profil verisini kaydetme işlemi (PUT isteği)
  const handleSave = async () => {
    ...
    try {
      const response = await axios.put(`${prodUrl}/customerauth/user/profile/update/${userId}`, updatedProfile, {
        headers: {
          Authorization: `Bearer ${accessToken}`
        }
      });
  ...
  // Input alanlarını kontrol et ve güncelle
